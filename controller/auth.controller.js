...
const getToken = async (req,res) => {
        try{
            const fToken = req.signedCookies.loginToken 
                // if(!fToken)
                //     return res.status(StatusCodes.NOT_FOUND).json({ msg: ` UnAuthorized, Cookie Not Found`}) //no need 
...
const loggedUser = async (req,res) => {
        try{
                let user = await User.findById({ _id: req.userId }).select('-password')
                    if(!user)
                        return res.status(StatusCodes.NOT_FOUND).json({ msg: `requsted user id not found `})
